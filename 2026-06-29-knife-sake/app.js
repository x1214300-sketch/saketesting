...
function StageRegionMap({ sake }) {
  const active = sake.regionMap || REGION_MAPS.find(m => m.id === sake.id) || REGION_MAPS[0];
  const mapTarget = active?.query || (active?.lat && active?.lng ? `${active.lat},${active.lng}` : `${sake.brewery} ${sake.region}`);
...
/* ================================================================
   主畫面：Intro
   ================================================================ */
function IntroScreen({ stores, eventName, sakeCount, onPick }) {
  return (
    <div className="px-5 py-8 fade-in">
      <div className="text-center mb-8">
        <p className="text-gold text-xs tracking-[0.4em] font-bold mb-2">SAKE TASTING 2026</p>
        <h1 className="text-4xl font-bold mb-3">{eventName || "清酒試飲會"}</h1>
        <p className="text-muted text-sm leading-relaxed">
          開瓶 · 品味 · 評鑑 · 議題討論<br/>
          一人一杯，{sakeCount} 款清酒的旅程
        </p>
      </div>
    ...
    <div className="fade-in px-5 pt-6 pb-12">
      ...
      {step === 2 && (
        <div className="space-y-3 mb-5">
          ...
          <div className="glass-panel p-4">
            <p className="text-xs font-bold text-gold tracking-widest mb-3">刀具購買意願</p>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={isSubmitting}
              onClick={submitOrder}
            >{isSubmitting ? "送出中..." : "提交採購單 ✓"}</button>
          </div>
          {submitError && (
            <div className="glass-panel p-4 border border-rose-200 bg-rose-50 text-rose-700 text-sm leading-relaxed">
              送出失敗，請先不要離開此頁，再按一次提交採購單。錯誤：{submitError}
            </div>
          )}
        </div>
      )}
...
function App({ data }) {
  // 從 localStorage 還原
  const eventId = data.meta?.eventId || "default";
  const restoredRaw = loadState();
  const restored = restoredRaw?.eventId === eventId ? restoredRaw : null;
  ...
  // 持久化
  useEffect(() => {
    saveState({ eventId, phase, venue, sakeIdx, evals, cart });
  }, [eventId, phase, venue, sakeIdx, evals, cart]);
  ...
  async function submitCart(extra) {
    ...
    const payload = {
      eventId: data.meta?.eventId || "default",
    ...
    if (endpoint) {
      ...
      if (!response.ok || result.status !== "ok") {
        throw new Error(result.error || `送出失敗：${response.status}`);
      }
      console.log("[sake order] orderId:", result.orderId);
    } else {
...
/* ================================================================
   啟動
   ================================================================ */
function getSakeDataUrl() {
  const params = new URLSearchParams(window.location.search);
  const requestedEvent = (params.get("event") || "").trim();
  const defaultEvent = (window.SAKE_DEFAULT_EVENT_ID || "").trim();
  const eventId = /^[a-z0-9_-]+$/i.test(requestedEvent)
    ? requestedEvent
    : (/^[a-z0-9_-]+$/i.test(defaultEvent) ? defaultEvent : "");

  return eventId ? `./events/${eventId}.json?v=2.6` : "./sake_data.json?v=2.6";
}

fetch(getSakeDataUrl())
  .then(r => r.json())
  .then(data => {
